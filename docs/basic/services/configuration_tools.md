...
- 理解"文本配置优先"的哲学，知道 GUI 只是生成文本的前端
- 对照三发行版网络配置方式（Netplan / systemd-networkd / nmcli）
- 会用 `timedatectl`、`hostnamectl`、`resolvectl` 完成时间、主机名、DNS 配置
- 知道改完配置如何验证生效，避免"改了但没 reload"
- 能在无图形界面的服务器上独立完成基础系统配置
