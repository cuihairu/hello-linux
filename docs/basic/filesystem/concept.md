...
文件系统是操作系统管理磁盘数据的方式——它规定数据按什么结构存放、如何命名、怎样被检索。但对用户来说，更重要的事实是：Linux 在具体文件系统之上盖了一层 **VFS（虚拟文件系统）**，让 ext4、XFS、Btrfs、NFS 甚至 `/proc` 这类"假目录"对你呈现完全一致的操作接口。理解这层抽象，是理解"为什么 mount 能把一块盘接到任意目录"和"为什么一切皆文件"的前提。
