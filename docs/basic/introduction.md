...
> Linux 是一个**内核**的名字，也常被用来指代"Linux 内核 + GNU 工具 + 包管理 + 配置"组成的整套**操作系统**。它 1991 年由 Linus Torvalds 开源发布，今天驱动着全球几乎全部超级计算机、绝大多数云服务器和绝大多数 Android 手机。你实际安装使用的是某个**发行版**——Debian/Ubuntu 系用 `apt` 装软件、RHEL/Rocky 系用 `dnf`、Arch 用 `pacman`；三者内核都是 Linux，差在软件仓库、默认配置和更新节奏。服务器与超算上 Linux 占绝对主导，桌面端约 4% 左右，主要使用者是开发者和技术爱好者。
