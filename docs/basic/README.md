...
2. **三系各装一台**。只学 Debian 会让你误以为 `apt` 是 Linux 标准，只学 Arch 会让你低估发行版的稳定性工作。最低配置建议：Ubuntu LTS（或 Debian stable）一台、Rocky Linux 一台、Arch Linux 一台，哪怕 Arch 只装在 20GB 的测试盘里。
