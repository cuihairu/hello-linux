...
- 一名用户在 `~/Downloads` 下载了 80 GB 数据集，分区瞬间告急；
...
另一类容易混淆的资源是 **inode**。文件系统格式化时就固定了 inode 总数，每个小文件都要消耗一个 inode。哪怕磁盘还有 100 GB 空闲，只要 inode 耗尽（典型场景：百万个几 KB 的小缓存文件），写入同样会失败。因此配额要同时限制块（KB）和 inode（文件数）两类指标。
...
宽限期（grace period）默认 7 天。一旦用量超过 soft 但未达 hard，系统开始计时；宽限期内可以继续写，超时后 soft 的效果等同于 hard，写入开始被拒绝，直到用量降回 soft 以下。这个设计的用意是容忍突发：平时写到 800 MB 无妨，只要在一周内清掉，就不会被卡死。把 soft 设为 0 表示"不设软限制，只用 hard"；hard 必须 ≥ soft，否则 `edquota` 保存时会拒绝。
...
表格数值单位是文件系统块（通常 1 块 = 1 KB），从左到右依次是已用块、块 soft、块 hard、已用文件数、inode soft、inode hard。`blocks` 已用量由内核实时统计写入，**不包含**你手工 `dd` 到块设备绕过文件系统的情况；改完配额数值立即生效，无需重启服务。

## 3. 启用配额：ext4（Debian/Ubuntu、Arch、Rocky 默认）

ext4 的配额靠内核在挂载时读取 `usrquota`/`grpquota` 选项启用，流程分三步：改挂载选项 → 重新挂载 → 运行 `quotacheck` 建库。**顺序不能颠倒**——`quotacheck` 需要在已启用配额的挂载点上扫描并创建 `aquota.user`/`aquota.group` 数据库文件，没先 remount 就执行会直接报错。三系安装工具包的命令不同（`apt install quota`、`pacman -S quota-tools`、`dnf install quota`），但之后的流程完全一致：
...
$ sudo quotacheck -cugm /home    # -c 创建数据库 -u 用户 -g 组 -m 禁止只读检查
$ sudo quotaon /home
$ sudo systemctl enable quotaon.service   # systemd 系发行版通用，实现开机自动开启
```
...
$ sudo xfs_quota -x -c 'limit -u alice 20g 30g' /data   # 软 20g，硬 30g，语法与 edquota 不同
...
```bash
$ quota                       # 当前登录用户查自己（无需 root），-s 以 GB/MB 显示
Disk quotas for user alice (uid 1000):
$ sudo quota -u alice         # 管理员查任意用户
$ sudo repquota /home         # 一次性审计整个挂载点，适合写进巡检脚本
*** Report for user quotas on /dev/sda2
alice    3145728 2097152 1048576   ...
```

`repquota` 输出的列依次是 hard、soft、已用量；`grace` 列出现倒计时（如 `6days+`）就意味着该用户已超软限制、正在宽限期内，是发提醒邮件的最佳时机。生产环境通常配合 cron 每天跑一次 `repquota -ugs` 并 diff 阈值。

## 6. 批量设置与复制

逐个 `edquota` 低效且容易输错，模板复制是批量初始化的正确姿势：先给一个"标准用户"调好配额存为模板，再用 `-p`（prototype）复制给多人；也可以按组套用。
...
6. **删除用户后残留配额记录**。`userdel -r` 不会自动清理 `aquota.user` 里的条目。长期运行的机器定期用 `repquota /home` 对比实际用户列表，或用 `edquota -d username`（若发行版支持）清理孤儿条目。
