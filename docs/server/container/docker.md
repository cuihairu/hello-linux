...
上线前逐条打勾——清单把前文长文压成可执行的最后一步，每一条对应的"为什么"都在前文出现过。

- [ ] 非 root 运行（`USER`）
- [ ] 资源限额（`--memory` / `--cpus`）
- [ ] 日志限额（`daemon.json` max-size / max-file）
- [ ] `--restart` 或 Compose `restart` 策略已设
- [ ] 镜像固定版本 tag，而非 `latest`
- [ ] 密钥不进镜像（环境变量注入或 secrets）
- [ ] `docker system df` 有例行清理任务
- [ ] 防火墙只放行确需的宿主机端口

安全基线的更完整清单见[加固篇](../../security/hardening.md)。记住容器安全的边界：镜像里的非 root、只读根、最小依赖管的是"容器被攻破后损失多大"，宿主机的防火墙、补丁与准入控制管的是"会不会被攻破"——两层缺一不可。真正成熟的团队，是让镜像、Compose、systemd 与防火墙规则各自待在版本库里，各司其职，谁也不越界替谁做决定；条目拿不准时回到对应章节重读"为什么"。
