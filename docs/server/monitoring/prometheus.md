...
Debian 的默认配置在 `/etc/prometheus/prometheus.yml`，数据目录 `/var/lib/prometheus/`，权限已随包交给 `prometheus` 用户——**不要**再手工 `chown` 无关目录，包管理器预设的属主就是升级不打架的前提；要改采集目标直接编辑 YAML 后 `systemctl reload prometheus`（或 `kill -HUP`）即可热加载 scrape 配置。
...
Packages (3) grafana-11.x  prometheus-2.5x  prometheus-node-exporter-1.8.x
(3/3) installing prometheus-node-exporter       [######################] 100%
$ sudo systemctl enable --now prometheus node_exporter grafana-server
$ pacman -Qi prometheus | head -5
Name            : prometheus
Version         : 2.5x.x
...
```yaml
# /etc/prometheus/rules/node_alerts.yml（Debian 片段风格；Arch 默认可放 /etc/prometheus/ 下并在主配置 rule_files 引用）
groups:
  - name: node_alerts
    rules:
      - alert: HighCPUUsage
