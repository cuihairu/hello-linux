...
教科书上的"北桥管内存与显卡、南桥管慢速外设"（下图）是正确的历史模型，但注意那行注释——**现代 CPU 已将内存控制器和大量 PCIe 根端口集成到内部，北桥作为独立芯片已消失**。这带来两个排障上的现实变化：内存带宽不再经过"前端总线"这个公共瓶颈（FSB 已淘汰）；而多路服务器上，CPU 与 CPU 之间的互联（Intel UPI / AMD Infinity Fabric）成了新的"总线"，也催生了 NUMA（见 [CPU 章](./cpu.md)）。
